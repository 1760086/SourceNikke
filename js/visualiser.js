...
function getCurrentSpineAssetUrls () {
      const id = currentid
      if (current_l2d === "fb") {
            return {
                  atlasUrl: "l2d/" + id + "/" + id + "_00.atlas",
...
function parseAtlasTexturePages (atlasText) {
      ...
      for (const line of atlasText.split(/\r?\n/)) {
            const t = line.trim()
            if (/^[^\s]+\.png$/i.test(t)) names.push(t)
      }
      return names
}
...
// One .zip download: .atlas + texture PNG(s) + .skel (extra PNGs if the atlas has multiple pages)
qs(".screenshot").addEventListener("click", async (e) => {
      if (!currentid) return
      if (typeof JSZip === "undefined") {
            alert("JSZip failed to load (js/jszip.min.js).")
            return
      }
      const urls = getCurrentSpineAssetUrls()
      if (!urls) return

      try {
            const zip = new JSZip()
            const atlasRes = await fetch(urls.atlasUrl)
            if (!atlasRes.ok) throw new Error("Atlas " + atlasRes.status)
            const atlasText = await atlasRes.text()
            const atlasName = urls.atlasUrl.split("/").pop()
            zip.file(atlasName, atlasText)

            const baseDir = urls.atlasUrl.slice(0, urls.atlasUrl.lastIndexOf("/") + 1)
            const pngNames = parseAtlasTexturePages(atlasText)
            if (pngNames.length === 0) {
                  throw new Error("No .png pages found in atlas")
            }

            for (const png of pngNames) {
                  const pngRes = await fetch(baseDir + png)
                  if (!pngRes.ok) throw new Error("Texture " + png + " " + pngRes.status)
                  zip.file(png, await pngRes.arrayBuffer())
            }

            const skelRes = await fetch(urls.skelUrl)
            if (!skelRes.ok) throw new Error("Skel " + skelRes.status)
            const skelName = urls.skelUrl.split("/").pop()
            zip.file(skelName, await skelRes.arrayBuffer())

            const zipBlob = await zip.generateAsync({ type: "blob" })
            const zipName = currentid + "_" + current_l2d + "_spine.zip"
            triggerDownloadBlob(zipBlob, zipName)
      } catch (err) {
            console.error(err)
            alert("Could not download Spine assets: " + (err && err.message ? err.message : err))
      }
})
